...
- Add the required global strategy and global-only movement-direction selectors to both configuration wizards as new Step 3. Add optional per-cover strategy overrides in a collapsible section; every selector defaults to a UI-only Use global setting choice that clears the override. Shift Max/Min Positions to Step 4, Tilt Angle Control to Step 5, Additional Settings and Window Sensors to Step 6, and Time Settings to Step 7.

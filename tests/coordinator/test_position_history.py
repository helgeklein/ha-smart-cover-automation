...
class TestPositionHistory:
    ...
    async def test_position_history_initialization(self, coordinator: DataUpdateCoordinator):
        ...
        history = coordinator._cover_pos_history_mgr.get("cover.non_existent")
        assert history == []
    ...
    async def test_position_history_single_update(self, coordinator: DataUpdateCoordinator):
        ...
        history = coordinator._cover_pos_history_mgr.get("cover.test")
        assert history == [50]
    ...
    async def test_position_history_multiple_covers(self, coordinator: DataUpdateCoordinator):
        ...
        history1 = coordinator._cover_pos_history_mgr.get("cover.test1")
        assert history1 == [50, 25]

        # Check history for second cover
        history2 = coordinator._cover_pos_history_mgr.get("cover.test2")
        assert history2 == [75]
    ...
    async def test_position_history_none_values(self, coordinator: DataUpdateCoordinator):
        ...
        coordinator._cover_pos_history_mgr.update("cover.test", None)
        coordinator._cover_pos_history_mgr.update("cover.test", 45)
        ...
        # Verify internal storage
        assert list(coordinator._cover_pos_history_mgr._cover_position_history["cover.test"]) == [45, None]
    ...
    async def test_get_newest_position_empty_history(self, coordinator: DataUpdateCoordinator):
        ...
        newest = coordinator._cover_pos_history_mgr.get_latest("cover.nonexistent")
    ...
    async def test_get_newest_position_single_entry(self, coordinator: DataUpdateCoordinator):
        ...
        newest = coordinator._cover_pos_history_mgr.get_latest("cover.test")
        assert newest == 75
    ...
    @pytest.mark.asyncio
    async def test_get_newest_position_with_none_values(self, coordinator: DataUpdateCoordinator):
        """Test getting newest position when None values are in history."""
        # Add positions including None
        coordinator._cover_pos_history_mgr.update("cover.test", 40)
        coordinator._cover_pos_history_mgr.update("cover.test", None)
        coordinator._cover_pos_history_mgr.update("cover.test", 80)

        # Should return the most recent one (80)
        newest = coordinator._cover_pos_history_mgr.get_latest("cover.test")
        assert newest == 80
        ...
        assert len(coordinator._cover_pos_history_mgr._cover_position_history["cover.test"]) == COVER_POSITION_HISTORY_SIZE

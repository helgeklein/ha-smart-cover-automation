...
class TestSmartReload:
    ...
    async def test_reload_with_external_morning_opening_time_change_only(
        self,
        mock_hass_with_spec,
        mock_config_entry_basic,
    ) -> None:
        """Changing only the external morning opening time should refresh, not reload."""

        mock_coordinator = MagicMock()
        mock_coordinator._merged_config = {
            "morning_opening_external_time": "08:00:00",
            "covers": ["cover.test"],
        }
        mock_coordinator.async_request_refresh = AsyncMock()
        ...
        mock_config_entry_basic.data = {}
        mock_config_entry_basic.options = {
            "morning_opening_external_time": "08:30:00",
            "covers": ["cover.test"],
        }
